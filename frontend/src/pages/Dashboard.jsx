...
function HomeView({ user, summary }) {
  const stats = [
    { label: "Today's Sales", value: `Rs ${(summary?.todaySales || 0).toFixed(0)}`, note: "Sales captured today" },
    { label: "Credit Due", value: `Rs ${(summary?.creditDue || 0).toFixed(0)}`, note: "Outstanding balance" },
      ...
      <div style={s.lowerGrid}>
        <div style={s.panel}>
          <div style={s.panelHeader}>
            <span style={s.panelTitle}>Recent Sales</span>
            <span style={s.panelLink}>{recentSales.length} entries</span>
          </div>
          <div style={s.tableList}>
            {recentSales.length === 0 ? (
              ...
              recentSales.map((sale) => (
                <div key={sale.id} style={s.row}>
                  <div style={s.rowMain}>
                    <div style={s.rowTitle}>{sale.customerName || sale.invoiceNo}</div>
                    <div style={s.rowMeta}>{sale.invoiceNo} - {sale.saleDate}</div>
                  </div>
                  <div style={s.rowAmount}>Rs {Number(sale.total || 0).toFixed(0)}</div>
                </div>
              ))
            )}
          </div>
        ...
        <div style={s.panel}>
          <div style={s.panelHeader}>
            <span style={s.panelTitle}>Operational Snapshot</span>
            <span style={s.panelLink}>Local</span>
          </div>
          <div style={s.snapshotList}>
            <SnapshotLine label="Products" value={summary?.productCount || 0} />
            <SnapshotLine label="Batches" value={summary?.batchCount || 0} />
            <SnapshotLine label="Suppliers" value={summary?.supplierCount || 0} />
            <SnapshotLine label="Mode" value="Offline-first" />
          </div>
        </div>
      </div>
    </>
    ...
    <div style={s.reportFrame}>
      <div style={s.reportCard}>
        ...
        <div style={s.reportGrid}>
          <SnapshotLine label="Sales total" value={`Rs ${(summary?.todaySales || 0).toFixed(0)}`} />
          <SnapshotLine label="Credit due" value={`Rs ${(summary?.creditDue || 0).toFixed(0)}`} />
