...
import { ReceiptText, RefreshCw, FileDown } from "lucide-react";
import { getReceivables } from "../../lib/posApi";
...
export default function ReceivablesWorkspace() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await getReceivables({ from, to, search, status: statusFilter, aging: agingFilter });
      setData(res);
    } catch (err) {
  ...
  }, [from, to, search, statusFilter, agingFilter]);
  ...
  const handleExport = () => {
    const wsData = [
      ["Invoice #", "Date", "Customer", "Total (Rs.)", "Paid (Rs.)", "Outstanding (Rs.)", "Aging", "Status"]
    ];
    data.rows.forEach(r => {
      wsData.push([
        r.invoice_no, r.sale_date, r.customer_name, r.total, r.amount_paid, r.balance_due, r.aging_bucket, r.payment_status
      ]);
    });
    wsData.push(["", "", "TOTAL", data.summary.total_billed || 0, data.summary.total_collected || 0, data.summary.total_outstanding || 0, "", ""]);
  ...
  const getStatusBadge = (status) => {
    return status === 'Unpaid' ? st.badge90Plus : st.badge1_30;
  };

  return (
    <div>
      <div style={{ ...st.pageTitle, marginBottom: 20 }}>
        <ReceiptText size={24} /> Receivables
      </div>

      <div style={{ ...st.cardWrapper, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)' }}>
        <div style={{ padding: 20, borderRight: '1px solid #c8d8c8' }}>
          <div style={st.cardLabel}>TOTAL BILLED</div>
          <div style={{ ...st.cardValue, ...st.valueNeutral }}>Rs. {Number(data.summary.total_billed || 0).toLocaleString()}</div>
        </div>
        <div style={{ padding: 20, borderRight: '1px solid #c8d8c8' }}>
          <div style={st.cardLabel}>TOTAL COLLECTED</div>
          <div style={{ ...st.cardValue, ...st.valueGreen }}>Rs. {Number(data.summary.total_collected || 0).toLocaleString()}</div>
        </div>
        <div style={{ padding: 20, borderRight: '1px solid #c8d8c8' }}>
          <div style={st.cardLabel}>OUTSTANDING</div>
          <div style={{ ...st.cardValue, ...st.valueAmber }}>Rs. {Number(data.summary.total_outstanding || 0).toLocaleString()}</div>
        </div>
      ...
      <div style={st.cardWrapper}>
        <div style={st.filterBar}>
          ...
          <input type="text" placeholder="Search customer..." value={search} onChange={e => setSearch(e.target.value)} style={st.input} />
              ...
              <tbody>
                {data.rows.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td style={st.tdStyle}>{r.sale_date}</td>
                    <td style={st.tdStyle}>{r.customer_name}</td>
                    ...
                    <td style={{ ...st.tdStyle, textAlign: 'right', fontFamily: 'monospace', color: r.balance_due > 0 ? '#c62828' : 'inherit' }}>{Number(r.balance_due).toLocaleString()}</td>
                    <td style={st.tdStyle}><span style={getAgingBadge(r.aging_bucket)}>{r.aging_bucket}</span></td>
                    <td style={st.tdStyle}><span style={getStatusBadge(r.payment_status)}>{r.payment_status}</span></td>
                  </tr>
                ))}
                <tr>
                  <td colSpan={3} style={{ ...st.tdStyle, ...st.totalsRow, textAlign: 'right' }}>TOTAL</td>
                  <td style={{ ...st.tdStyle, ...st.totalsRow, textAlign: 'right' }}>{Number(data.summary.total_billed || 0).toLocaleString()}</td>
                  <td style={{ ...st.tdStyle, ...st.totalsRow, textAlign: 'right' }}>{Number(data.summary.total_collected || 0).toLocaleString()}</td>
                  <td style={{ ...st.tdStyle, ...st.totalsRow, textAlign: 'right' }}>{Number(data.summary.total_outstanding || 0).toLocaleString()}</td>
                  <td colSpan={2} style={{ ...st.tdStyle, ...st.totalsRow }}></td>
                </tr>
              </tbody>
            </table>
          )}
