...
function CustomerHistoryView({ customer }) {
  ...
  const handleRecordPayment = async () => {
    ...
    try {
      ...
      setSuccessData({
        title: payModal?.general ? "Payment Recorded" : "Bill Payment Recorded",
        lines: [
          { label: "Customer", value: customer.name },
          ...(payModal?.invoiceRef ? [{ label: "Invoice", value: payModal.invoiceRef }] : []),
          { label: "Amount Paid", value: `Rs ${amount.toLocaleString()}`, mono: true },
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#6a8f6c", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                  {payModal.general ? "General Payment" : `Invoice - ${payModal.invoiceRef || ""}`}
                </div>
                <div style={{ fontSize: 17, fontWeight: 800, color: "#1b3a1d", marginTop: 2 }}>Record Payment</div>
              </div>
