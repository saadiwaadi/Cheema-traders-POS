...
function registerIpc() {
  const handlers = {
    ...
    "pos:suppliers:deletePayment": async (_, id) => store.deleteSupplierPayment(id),
    ...
    "pos:customers:deletePayment": async (_, id) => store.deleteCustomerPayment(id),
    "pos:customers:withdrawal": async (_, payload) => ({ withdrawal: await store.saveWithdrawal(payload) }),
    "pos:customers:deleteWithdrawal": async (_, id) => store.deleteCustomerWithdrawal(id),
    ...
    "pos:settings:get": () => store.getSettings(),
    "pos:settings:save": (_, payload) => store.updateSetting(payload.key, payload.value),
    "pos:settings:save_all": (_, payload) => store.saveSettings(payload),
    ...
    "pos:accounts:getLedger": (_, args) => store.getAccountLedger(args.accountId, args.startDate, args.endDate),
