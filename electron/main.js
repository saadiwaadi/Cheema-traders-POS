...
function registerIpc() {
  const handlers = {
    ...
    "pos:expenses:categories": () => store.listExpenseCategories(),
    "pos:expenses:save": (_, payload) => store.saveExpense(payload),
    "pos:expenses:delete": (_, id) => store.deleteExpense(id),
    "pos:books:cashBook": (_, args) => store.getCashBook(args || {}),
    "pos:books:ledger": (_, args) => store.getGeneralLedger(args || {}),
    "pos:books:accounts": () => store.getAccountsBalances(),
    "pos:analysis": (_, args) => store.getAnalysis(args || {}),
